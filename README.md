...
**Automated:** Add a **`PYPI_API_TOKEN`** repository secret whose value is a **PyPI API token** from [Account → API tokens](https://pypi.org/manage/account/token/) (scoped to **`streamtree`** or your publishing account). The token string must start with **`pypi-`**; a GitHub PAT, password, or truncated copy will fail the [PyPA publish action](https://github.com/pypa/gh-action-pypi-publish) with an invalid-token warning. When tests pass on a **`v*.*.*`** tag, the [release workflow](https://github.com/eddiethedean/streamtree/blob/main/.github/workflows/release.yml) runs lint, type check, pytest (including coverage), builds with **`uv build`**, publishes to **PyPI**, then publishes the same **`dist/`** artifacts to **GitHub Packages** in a separate job ( **`GITHUB_TOKEN`** + **`repository_owner`** as the registry username; **`packages: write`** on that job). Open the repository **Packages** page to browse published versions. If a tagged release fails at publish, fix the secret and **Re-run failed jobs** on that workflow run (no new tag needed if PyPI never accepted the upload). [Trusted publishing](https://docs.pypi.org/trusted-publishers/) avoids long-lived tokens once configured on PyPI and in the workflow.
